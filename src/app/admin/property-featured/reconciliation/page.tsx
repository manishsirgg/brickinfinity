...
export default function FeaturedReconciliationPage() {
  ...
  const recover = async (row: RazorpayPaymentRow) => {
    const response = await fetch('/api/admin/property-featured/reconcile/recover-missing-local-order', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ razorpay_order_id: row.order_id, razorpay_payment_id: row.payment_id, property_id: selectedPropertyId.trim(), plan_id: selectedPlanId }) });
  ...
  return <div className="max-w-7xl mx-auto p-8 space-y-6">
    ...
    {recoveringPaymentId && (() => { const row = scannerRows.find((r) => r.payment_id === recoveringPaymentId); if (!row) return null; return <section className="border rounded-xl p-4 bg-white space-y-3">
      ...
      <p className="text-sm text-orange-700">Use only after confirming which property this captured payment belongs to.</p>
