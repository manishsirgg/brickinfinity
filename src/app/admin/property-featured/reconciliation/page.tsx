...
export default function FeaturedReconciliationPage() {
  ...
  return <div className="max-w-7xl mx-auto p-8 space-y-6">
    ...
    <section className="border rounded-xl p-4 bg-white space-y-3">
      ...
      <div className="overflow-auto"><table className="w-full text-xs"><thead><tr className="border-b"><th className="p-2 text-left">Payment</th><th className="p-2 text-left">Amount</th><th className="p-2 text-left">Contact</th><th className="p-2 text-left">Local match</th><th className="p-2 text-left">Action</th></tr></thead><tbody>{visibleScannerRows.map((p)=><tr key={p.payment_id} className="border-b align-top"><td className="p-2">{p.payment_id}<div>{p.order_id || "-"}</div></td><td className="p-2">{(p.amount/100).toFixed(2)} {p.currency}<div>{p.status}</div></td><td className="p-2">{p.contact || "-"}<div>{p.email || "-"}</div></td><td className="p-2">{p.localOrderFound ? <div><div>{p.localOrderId} / {p.propertyId || '-'} ({p.localPaymentStatus || "-"}, {p.localActivationStatus || "-"})</div><span className={`px-2 py-1 rounded ${p.alreadyReconciled ? 'bg-green-100 text-green-700' : p.canReconcile ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>{p.label}</span></div> : p.canRecoverFromDetectedNotes ? <div><div>{p.detectedPropertyTitle || p.detectedPropertyId} / {p.detectedPlanName || p.detectedPlanKey || p.detectedPlanId}</div><span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded">Detected Property/Plan</span></div> : <span className="px-2 py-1 bg-orange-100 rounded">Needs Manual Review</span>}</td><td className="p-2">{p.canReconcile ? <button onClick={async () => { const response = await fetch('/api/admin/property-featured/reconcile', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ local_order_id: p.localOrderId, razorpay_order_id: p.order_id, razorpay_payment_id: p.payment_id, source: 'razorpay_scanner' }) }); const data = await response.json(); setResult(data); await fetchScanner(); await fetchLocalOrders(); }} className="px-2 py-1 rounded bg-blue-600 text-white">Reconcile</button> : p.canRecover ? <button onClick={() => { setRecoveringPaymentId(p.payment_id); setSelectedPropertyId(p.canRecoverFromDetectedNotes ? (p.detectedPropertyId || '') : ''); setSelectedPlanId(p.canRecoverFromDetectedNotes ? (plans.find((pl) => pl.plan_key === p.detectedPlanKey)?.id || p.detectedPlanId || '') : ''); setPropertyPreview(null); }} className={`px-2 py-1 rounded text-white ${p.canRecoverFromDetectedNotes ? 'bg-emerald-600' : 'bg-orange-600'}`}>{p.canRecoverFromDetectedNotes ? 'Recover Detected Order' : 'Recover Manually'}</button> : <span className="text-gray-500">Already Reconciled</span>}</td></tr>)}</tbody></table></div>
    </section>
