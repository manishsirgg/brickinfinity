...
export default async function OrderDetail({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  return <div className="max-w-5xl mx-auto p-8 space-y-6 text-sm">
    ...
    <section className="bg-white border rounded-xl p-4 space-y-2"><h2 className="font-semibold">Payment Details</h2><div>Local order id: {order.id}</div><div>Razorpay order id: {order.razorpay_order_id || "-"}</div><div>Razorpay payment id: {order.razorpay_payment_id || "-"}</div><div>Amount/Currency: {(order.amount_paise/100).toFixed(2)} {order.currency || "INR"}</div><div>Payment status: {order.payment_status || "-"}</div><div>Activation status: {order.activation_status || "-"}</div><div>Scheduled start: {formatAdminFinanceDateTime(order.featured_starts_at, "Scheduled start pending")}</div><div>Scheduled end: {formatAdminFinanceDateTime(order.featured_ends_at)}</div><div>Created at: {formatAdminFinanceDateTime(order.created_at)}</div><div>Paid at: {formatAdminFinanceDateTime(order.paid_at)}</div><div>Updated at: {formatAdminFinanceDateTime(order.updated_at)}</div><div>Failure reason: {order.failure_reason || "-"}</div><pre className="bg-gray-50 border rounded p-2 overflow-auto">{JSON.stringify(order.metadata || {}, null, 2)}</pre></section>
