    ...
    <div className="bg-white border rounded-xl overflow-auto">
      ...
      <tbody>
        ...
        {pageRows.map((o) => {
          ...
          return <tr key={o.id} className="border-b align-top"><td className="p-2">{new Date(o.created_at).toLocaleString()}<div className="text-gray-500">Paid: {o.paid_at ? new Date(o.paid_at).toLocaleString() : "-"}</div></td><td className="p-2">{o.users?.full_name || "-"}<div>{o.users?.email || "-"}</div><div>{o.users?.phone || "-"}</div></td><td className="p-2">{o.properties?.title || "-"}<div className="text-gray-500">{o.property_id || "-"}</div></td><td className="p-2">{o.plan_name || "-"}</td><td className="p-2">{formatInr(o.amount_paise)}<div>{o.currency || "INR"}</div></td><td className="p-2 space-y-1"><div className={badgeClass(paymentBadge[1])}>{paymentBadge[0]}</div><div><span className={badgeClass(activationBadge[1])}>{activationBadge[0]}</span></div>{String(o.activation_status).toLowerCase()==="scheduled" && <div className="text-slate-600">Scheduled from: {o.paid_at ? new Date(o.paid_at).toLocaleString() : "-"}</div>}{isStalePending && <div><span className={badgeClass("slate")}>Stale Pending</span></div>}{o.financeStatus === "stale_cancelled" && <div className="text-slate-600">Unpaid Razorpay order cancelled because a successful paid order already exists.</div>}{isStalePending && <div className="text-slate-600">This order is unpaid and older than {STALE_PENDING_MINUTES} minutes. It may be safely cancelled if no payment was completed.</div>}{o.financeStatus === "real_failed_payment" && <div className="text-red-600">{o.failure_reason || "Payment failed or verification failed."}</div>}</td><td className="p-2"><div>{o.razorpay_order_id || "-"}</div><div>{o.razorpay_payment_id || "-"}</div></td><td className="p-2 space-y-2"><Link className="underline block" href={`/admin/finance/property-featured/${o.id}`}>View details</Link>{isStalePending && <CancelStaleOrderButton orderId={o.id} className="underline text-red-700 disabled:opacity-50" successRedirect="/admin/finance?success=stale_cancelled" />}</td></tr>;
        })}
      </tbody></table>
    </div>
