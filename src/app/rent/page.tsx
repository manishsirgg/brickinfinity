...
export async function generateMetadata({
  ...
  const titleParts = ["Rent Properties"];
  ...
  return {
    ...
    description:
      "Browse verified rental homes with advanced filters including furnishing, tenant preference, and amenities.",
    alternates: {
      canonical: "/rent",
    },
  };
...
export default async function RentPage({
  ...
  const furnishing = cleanString(searchParams.furnishing);
  const preferredTenant = cleanString(searchParams.preferredTenant);
  ...
  if (preferredTenant)
    query = query.eq("preferred_tenant", preferredTenant);
    ...
    <main className="container-custom py-12 md:py-16 space-y-12">
      ...
      <div>
        ...
        <p className="text-sm text-muted mt-2">
          {count || 0} verified rentals available.
        </p>
      </div>
