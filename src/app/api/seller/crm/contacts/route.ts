...
const { data: properties } = propertyIds.length
  ? await ctx.supabase.from("properties").select("id,seller_id,title,slug,listing_type,property_type,price,hourly_rate,daily_rate,monthly_rate,status,verification_status,deleted_at").eq("seller_id", ctx.sellerId).in("id", propertyIds as string[]).is("deleted_at", null)
  : { data: [] as any[] };
...
const propMap = new Map(summaries.map((p:any)=>[p.id,p]));
const enriched = contacts.map((contact:any)=>{
  const propertySummary = propMap.get(getContactPropertyId(contact)) ?? null;
  if (!propertySummary && getContactPropertyId(contact)) {
    console.log("[seller-crm-property] summary attached", { contactId: contact.id, attached: false });
  }
  return { ...contact, property_summary: propertySummary };
});
