  ...
  return <div className="max-w-7xl mx-auto px-4 py-8 space-y-4">
    ...
    {loading ? <div className={card}>Loading...</div> : error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-700">{error}</div> : (
      <>
        ...
        {mode==="contacts" && <div className="space-y-3"><input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Search contacts" className="w-full md:w-96 border rounded-lg px-3 py-2"/>{contacts.length===0?<div className={card}>No contacts found.</div>:<div className="grid md:grid-cols-2 gap-3">{contacts.map((c:any)=><div className={card} key={c.id}><div className="font-semibold">{c.full_name}</div><div className="text-sm text-gray-600">{c.phone||c.email||"—"}</div><div className="text-xs mt-2">{humanize(c.contact_type)} • {humanize(c.lifecycle_stage)} • {humanize(c.lead_temperature)}</div><div className="text-xs">Budget: {formatCurrency(c.budget_min)} - {formatCurrency(c.budget_max)}</div><div className="text-xs">Next follow-up: {formatRelativeDueStatus(c.next_followup_at)}</div><div className="mt-3 flex gap-2 text-xs"><Link href={`/seller/crm/contacts/${c.id}`} className="border rounded px-2 py-1">View</Link></div></div>)}</div>}</div>}
        ...
        {mode==="followups" && <div className="space-y-2">{(data||[]).map((f:any)=><div key={f.id} className={card}><div className="font-semibold">{f.title}</div><div className="text-xs">{humanize(f.status)} • {humanize(f.priority)} • {humanize(f.channel)}</div><div className="text-xs">Due: {formatDateTime(f.due_at)} ({formatRelativeDueStatus(f.due_at)})</div></div>)}</div>}
        {mode==="activities" && <div className="space-y-2">{(data||[]).map((a:any)=><div key={a.id} className={card}><div className="font-semibold">{a.title}</div><div className="text-xs text-gray-600">{humanize(a.activity_type)} • {humanize(a.channel)} • {formatDateTime(a.created_at)}</div><div className="text-sm">{a.body||""}</div></div>)}</div>}
