...
const propId = (c:any)=>c?.metadata?.last_property_id||null;
...
export function SellerCrmClientPage({ title, mode="overview", id }: { title: string; subtitle?: string; mode?: Mode; id?: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setData(j.data); if (mode === "settings") setSettingsDraft(j.data?.settings ?? {});
      if (mode === "contact-detail") {
        ...
        setAux({notes:nj.data||[], followups:(fj.data||[]).filter((x:any)=>x.contact_id===id), deals:(dj.data||[]).filter((x:any)=>x.contact_id===id), timeline:aj.data||[]});
      }
  ...
  const followupBuckets = useMemo(()=>{ const now=Date.now(), today=(new Date()).toDateString(); const arr=data||[]; return {overdue:arr.filter((x:any)=>x.status==="scheduled"&&new Date(x.due_at).getTime()<now),today:arr.filter((x:any)=>x.status==="scheduled"&&new Date(x.due_at).toDateString()===today),upcoming:arr.filter((x:any)=>x.status==="scheduled"&&new Date(x.due_at).getTime()>now&&new Date(x.due_at).toDateString()!==today),completed:arr.filter((x:any)=>x.status==="completed"),cancelled:arr.filter((x:any)=>x.status==="cancelled")}; },[data]);
  ...
  return <div className="max-w-7xl mx-auto px-4 py-8 space-y-4">
    ...
    {!loading && mode==="overview" && <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[
        ["Total contacts", data?.counts?.total_contacts],["New contacts",data?.counts?.new_contacts],["Hot leads",data?.counts?.hot_leads],["Due today",data?.counts?.due_today_followups],["Overdue",data?.counts?.overdue_followups]
      ...
      <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-xl border bg-white p-4"><p className="font-medium mb-2">Recent contacts</p>{(data?.recent_contacts||[]).slice(0,5).map((c:any)=><div key={c.id}><Link className="underline" href={`/seller/crm/contacts/${c.id}`}>{c.full_name}</Link></div>)}</div><div className="rounded-xl border bg-white p-4"><p className="font-medium mb-2">Recent activities</p>{(data?.recent_activities||[]).slice(0,5).map((a:any)=><div key={a.id} className="text-sm">{a.title} • {formatDateTime(a.created_at)}</div>)}</div></div>
    </div>}

    {!loading && mode==="contact-detail" && data && <div className="space-y-3">
      <div className="border rounded p-3"><p className="font-semibold text-lg">{data.full_name}</p><p className="text-sm">{data.phone||"-"} • {data.email||"-"} • {data.whatsapp_number||"-"}</p><p className="text-sm">Source: {data.source||"-"} • Stage: {humanize(data.lifecycle_stage)} • Temp: {humanize(data.lead_temperature)}</p><p className="text-xs text-gray-500">Created {formatDateTime(data.created_at)} • Updated {formatDateTime(data.updated_at)}</p><div className="mt-2 flex gap-2 flex-wrap"><button className="underline" onClick={()=>logAndOpen(data,"call")}>Call</button><button className="underline" onClick={()=>logAndOpen(data,"whatsapp")}>WhatsApp</button><button className="underline" onClick={()=>logAndOpen(data,"email")}>Email</button>{propId(data)&&<Link className="underline" href={`/property/${propId(data)}`}>View Property</Link>}</div></div>
    ...
    {!loading && mode==="activities" && <div className="space-y-3"><div className="grid md:grid-cols-3 gap-2"><select className="border rounded p-2" value={activityFilters.activity_type} onChange={e=>setActivityFilters((s:any)=>({...s,activity_type:e.target.value}))}><option value="">All activity types</option>{["lead_created","call","whatsapp","email","note","stage_change","deal_created","deal_updated","followup_created","followup_completed","converted","lost","system"].map(x=><option key={x} value={x}>{humanize(x)}</option>)}</select><select className="border rounded p-2" value={activityFilters.channel} onChange={e=>setActivityFilters((s:any)=>({...s,channel:e.target.value}))}><option value="">All channels</option>{channels.map(x=><option key={x} value={x}>{humanize(x)}</option>)}</select></div>{(data||[]).length===0?<div className="rounded border p-3 text-sm text-gray-600">No activities found.</div>:(data||[]).map((a:any)=><div key={a.id} className="border rounded p-3 text-sm"><div className="font-medium">{a.title||humanize(a.activity_type)}</div><div>{humanize(a.activity_type)} • {humanize(a.channel)}</div><div>{a.body||"-"}</div><div>{a.contact_name||"Unknown contact"} {a.property_id&&<Link className="underline ml-2" href={`/property/${a.property_id}`}>Property</Link>}</div><div className="text-xs text-gray-500">{formatDateTime(a.created_at)}</div></div>)}</div>}
