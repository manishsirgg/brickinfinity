...
const propertyHref = (id?: string | null, slug?: string | null) => id ? (slug ? `/property/${id}/${slug}` : `/property/${id}`) : null;
  ...
  const action = async (url:string, method:string, body:any, success="Saved") => {
    ...
    try {
      ...
      if (!response.ok || json?.success === false || json?.ok === false) {
        ...
        const safeMessage = response.status === 404 && mode === "contact-detail"
