...
const propSlug = (c:any)=>safeMetadata(c?.metadata)?.last_property_slug||safeMetadata(c?.metadata)?.property_slug||null;
const propertyHref = (id?: string | null, slug?: string | null) => (id && slug) ? `/property/${id}/${slug}` : null;
...
setAux({notes:asArray(nj?.notes ?? nj?.data ?? nj), followups:asArray(fj?.followups ?? fj?.data ?? fj).filter((x:any)=>x?.contact_id===id), deals:asArray(dj?.deals ?? dj?.data ?? dj).filter((x:any)=>x?.contact_id===id), timeline:asArray(aj?.activities ?? aj?.data?.activities ?? aj?.data ?? aj)});
  ...
  return <div className="max-w-7xl mx-auto px-4 py-8 space-y-4">
    ...
    {!loading && mode==="contact-detail" && data && <div className="space-y-3">
      ...
      <div className="border rounded p-3"><p className="font-semibold text-lg">{safeText(data?.full_name, "Unnamed contact")}</p><p className="text-sm">{safeText(data?.phone, "")} • {safeText(data?.email, "")} • {safeText(data?.whatsapp_number, "")}</p><p className="text-sm">Source: {safeText(data?.source)} • Stage: {humanize(data?.lifecycle_stage ?? "new")} • Temp: {humanize(data?.lead_temperature ?? "cold")}</p><p className="text-xs text-gray-500">Created {formatDateSafe(data?.created_at)} • Updated {formatDateSafe(data?.updated_at)}</p><div className="mt-2 flex gap-2 flex-wrap"><button className="underline" onClick={()=>logAndOpen(data,"call")}>Call</button><button className="underline" onClick={()=>logAndOpen(data,"whatsapp")}>WhatsApp</button><button className="underline" onClick={()=>logAndOpen(data,"email")}>Email</button>{propertyHref(propId(data), propSlug(data))&&<Link className="underline" href={propertyHref(propId(data), propSlug(data))!}>View Property</Link>}</div></div>
