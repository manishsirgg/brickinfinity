...
setAux({notes:asArray(nj?.notes ?? nj?.data ?? nj), followups:asArray(fj?.followups ?? fj?.data ?? fj).filter((x:any)=>x?.contact_id===id), deals:asArray(dj?.deals ?? dj?.data ?? dj).filter((x:any)=>x?.contact_id===id), timeline:asArray(aj?.activities ?? aj?.data?.activities ?? aj?.data ?? aj)});
  ...
  return <div className="max-w-7xl mx-auto px-4 py-8 space-y-4">
    ...
    {!loading && mode==="contact-detail" && data && <div className="space-y-3">
      ...
      <div className="border rounded p-3"><p className="font-medium mb-2">Follow-ups</p><button className="border rounded px-2 py-1 text-sm" onClick={()=>{const title=prompt("Title")||"Follow-up"; action("/api/seller/crm/followups","POST",{title,description:"",due_at:new Date().toISOString(),priority:"medium",channel:"call",contact_id:id},"Follow-up created");}}>Create follow-up</button>{(aux.followups||[]).map((f:any)=><div key={f.id} className="border rounded p-2 mt-2 text-sm">{f.title} • {formatDateSafe(f?.due_at)} • {f.channel} • {f.priority}<div className="flex gap-2"><button className="underline" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"completed"},"Follow-up completed")}>Complete</button><button className="underline" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"cancelled"},"Follow-up cancelled")}>Cancel</button><button className="underline" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"missed"},"Follow-up marked missed")}>Missed</button></div></div>)}</div>
    ...
    {!loading && mode==="followups" && <div className="grid gap-3 md:grid-cols-2">{Object.entries(followupBuckets).map(([k,items])=><div key={k} className="border rounded p-3"><p className="font-semibold mb-2">{humanize(k)}</p>{(items as any[]).map(f=><div key={f.id} className="border rounded p-2 text-sm mb-2">{f.title} • {formatDateSafe(f?.due_at)} • {f.channel} • <SellerCrmStatusBadge value={f.status}/><div className="flex gap-2"><button className="underline" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"completed"})}>Complete</button><button className="underline" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"cancelled"})}>Cancel</button><button className="underline" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"missed"})}>Missed</button></div></div>)}</div>)}</div>}
