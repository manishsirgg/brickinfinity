...
function CrmPanel({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <section className={`rounded-2xl border border-slate-200/80 bg-white p-4 sm:p-5 shadow-sm ${className}`}>{children}</section>;
}
function SectionHeader({ title, subtitle }: { title: string; subtitle?: string }) {
  return <div className="mb-3"><h3 className="text-base font-semibold text-slate-900">{title}</h3>{subtitle ? <p className="text-xs text-slate-500 mt-1">{subtitle}</p> : null}</div>;
}
function CrmActionButton({ children, onClick, href, variant = "default" }: { children: ReactNode; onClick?: () => void; href?: string; variant?: "default"|"danger"|"muted" }) {
  const base = "inline-flex items-center justify-center rounded-xl border px-3 py-2 text-xs font-medium transition min-h-9";
  const tone = variant === "danger" ? "border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100" : variant === "muted" ? "border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100" : "border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100";
  if (href) return <Link href={href} className={`${base} ${tone}`}>{children}</Link>;
  return <button type="button" onClick={onClick} className={`${base} ${tone}`}>{children}</button>;
}
function EmptyState({ title, description }: { title: string; description: string }) {
  return <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/70 p-4 text-sm"><p className="font-medium text-slate-700">{title}</p><p className="mt-1 text-slate-500">{description}</p></div>;
}
...
setAux({notes:asArray(nj?.notes ?? nj?.data ?? nj), followups:asArray(fj?.followups ?? fj?.data ?? fj).filter((x:any)=>x?.contact_id===id), deals:asArray(dj?.deals ?? dj?.data ?? dj).filter((x:any)=>x?.contact_id===id), timeline:asArray(aj?.activities ?? aj?.data?.activities ?? aj?.data ?? aj)});
  ...
  return <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8 space-y-5 overflow-x-hidden">
    ...
    {okMsg && <div role="status" className="rounded-xl border border-green-200 bg-green-50 p-3 text-sm text-green-700">{okMsg}</div>}
    ...
    {!loading && mode==="overview" && <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-xl border bg-white p-4"><p className="font-medium mb-2">Recent contacts</p>{asArray(data?.recentContacts).length===0?<EmptyState title="No contacts yet" description="New property enquiries will automatically appear here." />:asArray(data?.recentContacts).slice(0,5).map((c:any, idx:number)=><div key={c?.id ?? `recent-contact-${idx}`} className="rounded-lg border border-slate-100 bg-slate-50 p-2 mb-2"><Link className="font-medium text-slate-800 hover:underline" href={`/seller/crm/contacts/${c?.id}`}>{safeText(c?.full_name, "Unnamed contact")}</Link></div>)}</div><div className="rounded-xl border bg-white p-4"><p className="font-medium mb-2">Recent activities</p>{asArray(data?.recentActivities).length===0?<EmptyState title="No activity yet" description="Calls, notes and follow-ups will appear once your team starts engaging contacts." />:asArray(data?.recentActivities).slice(0,5).map((a:any, idx:number)=><div key={a?.id ?? `recent-activity-${idx}`} className="mb-2 rounded-lg border border-slate-100 bg-slate-50 p-2 text-sm">{safeText(a?.title)} • {formatDateSafe(a?.created_at)}</div>)}</div></div>
    </div>}

    {!loading && mode==="contact-detail" && data && <div className="space-y-3">

      <div className="border rounded p-3"><p className="font-semibold text-lg">{safeText(data?.full_name, "Unnamed contact")}</p><p className="text-sm">{safeText(data?.phone, "")} • {safeText(data?.email, "")} • {safeText(data?.whatsapp_number, "")}</p><p className="text-sm">Source: {safeText(data?.source)} • Stage: {humanize(data?.lifecycle_stage ?? "new")} • Temp: {humanize(data?.lead_temperature ?? "cold")}</p><p className="text-xs text-gray-500">Created {formatDateSafe(data?.created_at)} • Updated {formatDateSafe(data?.updated_at)}</p><div className="mt-2 grid grid-cols-2 sm:flex gap-2 flex-wrap"><CrmActionButton onClick={()=>logAndOpen(data,"call")}>Call</CrmActionButton><CrmActionButton onClick={()=>logAndOpen(data,"whatsapp")}>WhatsApp</CrmActionButton><CrmActionButton onClick={()=>logAndOpen(data,"email")}>Email</CrmActionButton>{propertyHref(data) ? <CrmActionButton href={propertyHref(data)!}>View Property</CrmActionButton> : getContactPropertyId(data) ? <span className="text-gray-500">Property link unavailable</span> : null}</div></div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-sm">
        ...
        {getContactPropertyId(data) ? <>
          ...
          {propertyHref(data) ? <CrmActionButton href={propertyHref(data)!}>View Property</CrmActionButton> : <span className="text-gray-500">Property link unavailable</span>}
      ...
      <div className="border rounded p-3"><p className="font-medium mb-2">Follow-ups</p><button className="border rounded px-2 py-1 text-sm" onClick={()=>{const title=prompt("Title")||"Follow-up"; action("/api/seller/crm/followups","POST",{title,description:"",due_at:new Date().toISOString(),priority:"medium",channel:"call",contact_id:id},"Follow-up created");}}>Create follow-up</button>{(aux.followups||[]).map((f:any)=><div key={f.id} className="border rounded p-2 mt-2 text-sm">{f.title} • {formatDateSafe(f?.due_at)} • {f.channel} • {f.priority}<div className="mt-1">Status: <SellerCrmStatusBadge value={f.status}/>{f.status==="completed"&&f?.completed_at?` • Completed ${formatDateSafe(f.completed_at)}`:""}{f.status==="cancelled"&&f?.cancelled_at?` • Cancelled ${formatDateSafe(f.cancelled_at)}`:""}{f.status==="missed"&&f?.missed_at?` • Missed ${formatDateSafe(f.missed_at)}`:""}</div>{f.status==="scheduled"&&<div className="flex gap-2"><CrmActionButton onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"completed"},"Follow-up completed")}>Complete</CrmActionButton><CrmActionButton variant="muted" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"cancelled"},"Follow-up cancelled")}>Cancel</CrmActionButton><CrmActionButton variant="danger" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"missed"},"Follow-up marked missed")}>Missed</CrmActionButton></div>}</div>)}</div>
      <div className="border rounded p-3"><p className="font-medium mb-2">Deals</p><button className="border rounded px-2 py-1 text-sm" onClick={()=>action("/api/seller/crm/deals","POST",{contact_id:id},"Deal created")}>Create deal</button>{(aux.deals||[]).map((d:any)=><div key={d.id} className="text-sm mt-2 border rounded p-2"><div className="font-medium"><Link className="underline" href={`/seller/crm/deals/${d.id}`}>{safeText(d.title, "New Property Deal")}</Link></div><div className="mt-1 flex gap-2 items-center flex-wrap"><select className="border" value={d.deal_stage} onChange={e=>action(`/api/seller/crm/deals/${d.id}`,"PATCH",{deal_stage:e.target.value},"Deal stage updated")}>{SELLER_CRM_DEAL_STAGES.map(s=><option key={s} value={s}>{formatSellerCrmDealStage(s)}</option>)}</select><span>{d.expected_value?inr(d.expected_value):""}</span>{(d?.property_summary?.href||propertyHref(d))?<Link className="underline" href={d?.property_summary?.href||propertyHref(d)!}>View Property</Link>:null}</div><div className="text-gray-600">{formatSellerCrmDealStage(d.deal_stage)}</div></div>)}</div>
    </div>}


    {!loading && mode==="contacts" && <div className="space-y-3">{asArray(data).length===0?<EmptyState title="No contacts found" description="Try adjusting filters or wait for new enquiries to sync into CRM." />:asArray(data).map((c:any)=><div key={c?.id} className="rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-sm"><div className="font-medium"><Link className="underline" href={`/seller/crm/contacts/${c?.id}`}>{safeText(c?.full_name, "Unnamed contact")}</Link></div><div>{safeText(c?.phone, "—")} • {humanize(c?.lifecycle_stage ?? "new")} • {humanize(c?.lead_temperature ?? "cold")}</div>{getContactPropertyId(c)&&<div className="mt-1">{safeText(c?.property_summary?.title || c?.property_summary?.property_type, "Linked property")} {propertyHref(c)?<Link className="underline ml-2" href={propertyHref(c)!}>View Property</Link>:<span className="text-gray-500 ml-2">Property link unavailable</span>}</div>}</div>)}</div>}
    {!loading && mode==="followups" && (
      <div className="grid gap-3 md:grid-cols-2">
        {Object.entries(followupBuckets).map(([k, items]) => (
          <div key={k} className="border rounded p-3">
            <p className="font-semibold mb-2">{humanize(k)}</p>
            {(items as any[]).length === 0 ? (
              <EmptyState title={`No ${humanize(k)} follow-ups`} description="Everything looks clear in this bucket." />
            ) : (
              (items as any[]).map((f:any) => (
                <div key={f.id} className="border rounded p-2 text-sm mb-2">
                  {f.title} • {formatDateSafe(f?.due_at)} • {f.channel} • <SellerCrmStatusBadge value={f.status} />
                  {f.status==="completed"&&f?.completed_at?` • Completed ${formatDateSafe(f.completed_at)}`:""}
                  {f.status==="cancelled"&&f?.cancelled_at?` • Cancelled ${formatDateSafe(f.cancelled_at)}`:""}
                  {f.status==="missed"&&f?.missed_at?` • Missed ${formatDateSafe(f.missed_at)}`:""}
                  {f.status==="scheduled" && (
                    <div className="mt-2 flex gap-2 flex-wrap">
                      <CrmActionButton onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"completed"},"Follow-up completed")}>Complete</CrmActionButton>
                      <CrmActionButton variant="muted" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"cancelled"},"Follow-up cancelled")}>Cancel</CrmActionButton>
                      <CrmActionButton variant="danger" onClick={()=>action(`/api/seller/crm/followups/${f.id}`,"PATCH",{status:"missed"},"Follow-up marked missed")}>Missed</CrmActionButton>
                    </div>
                  )}
                </div>
              ))
